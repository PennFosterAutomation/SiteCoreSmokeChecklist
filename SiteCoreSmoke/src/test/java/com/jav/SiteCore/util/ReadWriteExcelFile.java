...
public class ReadWriteExcelFile {
	...
	public String[][] readExcelFileWithSpaces(String sheetName , int columcount) {
		 HSSFSheet sheet = workbook.getSheet(sheetName);
		 Iterator rows = sheet.rowIterator();
		 int numberOfRow = sheet.getLastRowNum()- sheet.getFirstRowNum();
		 String[][] array = new String[numberOfRow+1][columcount];
		 int i =0;
	     while (rows.hasNext()){
...
// ###################  Custom ##############################
	...
	public String[][] removeRowFrom2dArray(String[][] array, int row){
		  ...
		  int p = 0;
		  int q ;
		     String[][] arrayToReturn = new String[rows -1][array[0].length];
		     for(i = 0; i < rows; i++){
		      q=0;
		      if(i == row){
		       continue;
		      ...
		      for(j = 0;j<array[0].length;j++){
		       arrayToReturn[p][q] = array[i][j];
		       System.out.println(arrayToReturn[p][q]);
		       q++;
		      }
		      p++;
		     }
	...
	@SuppressWarnings("rawtypes")
	public String[][] readExcelFileWithSpacesinSearch(String sheetName) {
		 HSSFSheet sheet = workbook.getSheet(sheetName);
		 Iterator rows = sheet.rowIterator();
		 int numberOfRow = sheet.getLastRowNum()- sheet.getFirstRowNum();
		 String[][] array = new String[numberOfRow+1][1];
		 int i =0;
	     while (rows.hasNext()){
		       HSSFRow row = ((HSSFRow) rows.next());
		       Iterator cells = row.cellIterator();
